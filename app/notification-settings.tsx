...
// Welche Toggles pro Rolle sichtbar sind
type ToggleKey = keyof Omit<NotificationSettings, "pushEnabled">;
...
function getTogglesForRole(role: string): ToggleDef[] {
  if (role === "admin" || role === "office") {
    return [
      { key: "applicationStatus", labelKey: "notifSettings.adminNewApplications", subtitleKey: "notifSettings.adminNewApplicationsSub" },
      { key: "assignments", labelKey: "notifSettings.adminPendingAssignments", subtitleKey: "notifSettings.adminPendingAssignmentsSub" },
  ...
  return [
    { key: "chatMessages", labelKey: "notifSettings.chatMessages", subtitleKey: "notifSettings.chatMessagesSub" },
    { key: "assignments", labelKey: "notifSettings.menteeMentorAssigned", subtitleKey: "notifSettings.menteeMentorAssignedSub" },
    { key: "feedback", labelKey: "notifSettings.menteeFeedback", subtitleKey: "notifSettings.menteeFeedbackSub" },
  ];
}
...
export default function NotificationSettingsScreen() {
  ...
  const toggles = getTogglesForRole(user.role);
    ...
    <Container fullWidth={Platform.OS === "web"}>
      ...
      >
        ...
        <View style={styles.page}>
          <Text style={[styles.subtitle, { color: themeColors.textSecondary }]}>{t("notifSettings.subtitle")}</Text>

          {/* Master-Toggle */}
          <Text style={[styles.sectionLabel, { color: themeColors.textTertiary }]}>{t("notifSettings.pushMasterLabel")}</Text>
          ...
          {/* Rollenspezifische Toggles */}
          <Text style={[styles.sectionLabel, { color: themeColors.textTertiary }]}>{t("notifSettings.categoryLabel")}</Text>
          <View style={[styles.card, { backgroundColor: themeColors.card, borderColor: themeColors.border }]}>
            {toggles.map((td, idx) => (
              <React.Fragment key={td.key}>
                {idx > 0 && <View style={[styles.divider, { backgroundColor: themeColors.border }]} />}
                <ToggleRow
                  label={t(td.labelKey as any)}
                  subtitle={t(td.subtitleKey as any)}
                  value={settings[td.key]}
                  onToggle={() => toggle(td.key)}
                  themeColors={themeColors}
                  isLast={idx === toggles.length - 1}
                />
              </React.Fragment>
            ))}
          </View>
...
function ToggleRow({ label, subtitle, value, onToggle, themeColors, isLast }: {
  label: string;
  subtitle: string;
  value: boolean;
  onToggle: () => void;
  ...
  return (
    <View style={[styles.toggleRow, isLast && styles.toggleRowLast]}>
      ...
      <Switch value={value} onValueChange={onToggle} trackColor={{ false: themeColors.border, true: COLORS.gradientStart }} thumbColor={COLORS.white} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  saveButton: { borderRadius: 10, paddingVertical: 14, alignItems: "center" },
